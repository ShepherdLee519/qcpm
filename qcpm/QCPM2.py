...
class QCPatternMapper2:
    """ Quantum circuit pattern mapper Class.

    Example:
        QCPM2 = QCPatternMapper2()

        # 1. single file to file
        QCPM2.execute(circuit_path, './circuit_after')

        # 2. dir to dir (batch work)
        QCPM2.execute('./data/', './output/', strategy='MCM')
    ...
    def __init__(self, **kwargs):
        ...
        with logging(self.log, 'w'):
            self.patternMapper = Mapper('pattern')
            self.commuMapper = Mapper('commutation')
            self.hadamMapper = Mapper('hadamard')
            self.reverMapper = Mapper('reversible')

    def _optimize(self, circuit, **kwargs):
        iteration = 3
        ...
        while count < iteration:
            ...
            self.reverMapper.execute(circuit, 
                    system=system_input, strategy=strategy, metric=metric)
            self.hadamMapper.execute(circuit, 
                    system=system_input, strategy=strategy, metric=metric)
            self.commuMapper.execute(circuit, 
                    system=system_input, strategy=strategy, metric=metric)

            changed = circuit.draft != draft
            if not changed:
    ...
    def execute(self, input_path, output_path='', **kwargs):
        ...
        with logging(self.log):
            with timer:
                ...
                circuit = Circuit(input_path, system=system_input, optimize=False)
                if optimize:
                    self._optimize(circuit, system=system_input, strategy=strategy, metric=metric)

                # after loading circuit, check the depth size
                ...
                changed = self.patternMapper.execute(circuit, 
                    system=system_input, strategy=strategy, metric=metric)

                while changed and turn < LIMIT:
                    optimize and self._optimize(circuit, 
                        system=system_input, strategy=strategy, metric=metric)
